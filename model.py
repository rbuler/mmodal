    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
                                        nn.Linear(self.L * self.K,
                                                  num_classes))        ## 1 for baseline, self.D  for multimodal
...
class EarlyFusionModel(nn.Module):
    def __init__(self, tab_dim, rad_dim, device, hidden_dim=128, out_dim=1):
        super(EarlyFusionModel, self).__init__()

        self.mil = GatedAttentionMIL()

        self.rad_norm = nn.LayerNorm(rad_dim)
    ...
    def forward(self, image, tabular_feat, radiomics_feat, mask_feat):

        instances, instances_ids, _ = self.patcher.convert_img_to_bag(image.squeeze(0))
        ...
        # x = torch.cat([image_feat, tabular_feat, rad_normed, mask_feat], dim=1)
        # x = F.relu(self.fc1(x))
        # x = F.relu(self.fc2(x))
        # x = self.fc3(x)  # logits
        return x
